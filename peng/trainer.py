...
class Trainer(object):
    ...
    def order_sample(self, batch_x, batch_y, sample_num):
        ...
        def get_order(card):
            ...
            indexes = [i for i in range(card)]
            orders_1 = [tuple(indexes)]

            for i in range(len(indexes)-1):
                for j in range(i, len(indexes)):
                    new_lis = swap(indexes, i, j)
                    if tuple(new_lis) not in orders_1:
                        orders_1.append(tuple(new_lis))
            ...
            for order in orders_1:
                ...
                for i in range(len(order)-1):
                    for j in range(i, len(order)):
                        new_lis = swap(order, i, j)
                        if tuple(new_lis) not in orders_1:
                            orders_2.append(tuple(new_lis))
                            
            orders = orders_2
            if len(orders) < sample_num:
                tmp = []
                for i in range(math.ceil(sample_num / len(orders))):
                    tmp += orders
                orders = tmp
            return orders

        def get_card(y):
            # print([1 if i != 1 else 0 for i in y ])
            tuple_num = int((sum([1 if i != 1 else 0 for i in y ]) - 1) / 5)
            return tuple_num

        def reorder(x, orders):
            lis = []
            for order in orders:
                tmp = copy.deepcopy(x)
                for i in range(len(order)):
                    tmp[1 + i * 5 : 1 + i * 5 + 5] = x[1 + order[i] * 5 : 1 + order[i] * 5 + 5]
                lis.append(tmp)
            return lis
            
        def map_large_integers_to_string(arr):
            unique_sorted_elements = sorted(set(arr))
            char_mapping = {}
            for i, val in enumerate(unique_sorted_elements):
                if i < 10:
                    char_mapping[val] = str(i)  # 0-9
                elif i < 37:
                    char_mapping[val] = chr(ord('a') + i - 10)  # 10-35 -> a-z
                else:
                    char_mapping[val] = str(0)
            
            mapped_string = ''.join([char_mapping[val] for val in arr])
        ...
        new_tgt_tokens = []
        # print("==================================")
        for y in batch_y['tgt_tokens']:
            # print(y)
            card = get_card(y)
            orders = get_order(card)
            new_tgt_token = reorder(y, orders)
            orders_with_distances = [
                (orders[i], levenshtein_distance(map_large_integers_to_string(new_tgt_token[0].tolist()), map_large_integers_to_string(new_tgt_token[i].tolist())))
                for i in range(len(new_tgt_token))
            ]
            orders_with_distances.sort(key=lambda x: x[1])
            orders = [orders_with_distances[i][0] for i in range(sample_num)]
            new_tgt_tokens += reorder(y, orders)

        torch.set_printoptions(threshold=np.inf)
        for key, value in batch_x.items():
            if key == 'tgt_tokens':
                new_batch_x[key] = torch.stack(new_tgt_tokens)
                # print(new_batch_x[key])
            elif key == 'target_span':
                tmp = []
                target_span = np.concatenate([value for i in range(sample_num)])
                for j in range(0, len(value)):
                    for i in range(j, len(target_span), len(value)):
                        tmp.append(target_span[i])
                new_batch_x[key] = np.concatenate(tmp)
                # print(new_batch_x[key])
            else:
                try:
                    others = torch.from_numpy(np.concatenate([value for i in range(sample_num)]))

                    tmp = []
                    for j in range(0, len(value)):
                        for i in range(j, len(others), len(value)):
                            tmp.append(others[i])
                    new_batch_x[key] = torch.stack(tmp)
                    # print(new_batch_x[key])
                except:
                    others = torch.from_numpy(np.concatenate([value for i in range(sample_num)]))
                    tmp = torch.zeros(others.shape, dtype=torch.int)
                    count = 0
                    for j in range(0, len(value)):
                        for i in range(j, len(others), len(value)):
                            tmp[count] = others[i]
        ...
        for key, value in batch_y.items():
            if key == 'tgt_tokens':
                new_batch_y[key] = torch.stack(new_tgt_tokens)
            elif key == 'target_span':
                tmp = []
                target_span = np.concatenate([value for i in range(sample_num)])
                for j in range(0, len(value)):
                    for i in range(j, len(target_span), len(value)):
                        tmp.append(target_span[i])
                new_batch_y[key] = np.concatenate(tmp)
            else:
                try:
                    others = torch.from_numpy(np.concatenate([value for i in range(sample_num)]))
                    ...
                    for j in range(0, len(value)):
                        for i in range(j, len(others), len(value)):
                            tmp.append(others[i])
                    new_batch_y[key] = torch.stack(tmp)
                except:
                    others = torch.from_numpy(np.concatenate([value for i in range(sample_num)]))
                    tmp = torch.zeros(others.shape, dtype=torch.int)
                    count = 0
        ...
        with inner_tqdm(total=self.n_steps, postfix='loss:{0:<6.5f}', leave=False, dynamic_ncols=True,
            ...
            for epoch in range(self.epoch, self.n_epochs + 1):
                ...
                for batch_x, batch_y in self.data_iterator:
                    batch_x, batch_y = self.order_sample(batch_x, batch_y, sample_num=6)
                    self.step += 1
