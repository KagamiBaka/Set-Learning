...
def _beam_search_generate(decoder: Seq2SeqDecoder, tokens=None, state=None, max_length=20, max_len_a=0.0, num_beams=4,
    ...
    def is_same_set(hypothesis1, hypothesis2):
        set1 = set()
        set2 = set()
        for i in range(1, len(hypothesis1) - 4, 5):
            if hypothesis1[i: i + 5] not in set1:
                set1.add(hypothesis1[i: i + 5])

        for i in range(1, len(hypothesis2) - 4, 5):
            if hypothesis2[i: i + 5] not in set2:
                set2.add(hypothesis2[i: i + 5])
        if set1 == set2:
            if len(set1) != 0:
            ...
            # print(set1, set2)
            return False
    # 这里需要重新写，需要写一个合并hypos的函数，不同排序但表示同一个集合的序列需要进行概率的合并
    def merge_hypotheses(hypotheses):
        merged = []
        for hypothesis in hypotheses:
            in_set = False
            for i in range(len(merged)):
                if is_same_set(hypothesis[1], merged[i][1]):
                    in_set = True
                    merged[i] = (math.log(math.exp(merged[i][0]) + math.exp(hypothesis[0])), merged[i][1])
                    break
            if in_set == False:
    ...
    for i, hypotheses in enumerate(hypos):
        len1 = len(hypotheses.hyp)
        hypotheses.hyp = merge_hypotheses(hypotheses.hyp)
        len2 = len(hypotheses.hyp)
        # print(len1 - len2)
